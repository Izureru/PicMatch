...
namespace PicMatch
{
    ...
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            Console.WriteLine("{0}\n{1}\n{2}", matchings12[0].Similarity, matchings13[0].Similarity,
                matchings14[0].Similarity);

            // compare similarity levels and return result
            if (matchings12[0].Similarity > matchings13[0].Similarity &&
                matchings12[0].Similarity > matchings14[0].Similarity)
            {
                Console.WriteLine("Image 2 is best match");
            }

            if (matchings13[0].Similarity > matchings12[0].Similarity &&
                matchings13[0].Similarity > matchings14[0].Similarity)
            {
